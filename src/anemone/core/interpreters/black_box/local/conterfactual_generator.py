"""
Get the smallest changein input to get the 'target' value as model output
"""
...
class CounterfactualExplaination(BaseExplaination):
    """
    The cnunterfactual explaination wrapper
    """


class CounterfactualGenerator(BaseInterpreter):
    """
    Interpreter that from a target and an input,
    returns an object with same signature of input but predicted as target from the model
    """

    def run(self):

        parsed__query = sqlglot.parse_one(self.context.selection)
        parsed__query_select = parsed__query.find(sqlglot.expressions.Select)
        if parsed__query_select is None:
            raise ValueError("Impossible to get the select part of the query")
        parsed__query_select.args["expressions"] = [
            sqlglot.expressions.Column(this=name) for name in self.context.dataset.get_features_names()
        ]
        features_query = parsed__query_select.sql()
        parsed__query_select.args["expressions"] = [
            sqlglot.expressions.Column(this=name) for name in self.context.dataset.get_targets_names()
        ]
        target_query = parsed__query_select.sql()

        x = self.context.dataset.select(features_query)
        y = self.context.dataset.select(target_query)
        alogithm = NSGA2(**self.context.config)
        result = alogithm.run(lambda x: self.context.model.predict(x) - y, x.shape[1:], -1, 1)
        return CounterfactualExplaination(result, metadata=None)

    @staticmethod
    def get_config_signature():
        return ConfigSignature(config={"number_of_examples": Scalar()})


class NSGA2:
    """A class to implement the NSGA-II multi-objective optimization algorithm"""

    def __init__(self, max_iter=100, pop_size=100, p_crossover=0.7, alpha=0, p_mutation=0.2, mu=0.02, verbose=False):
    ...
    def run(self, cost_function, var_size, var_min, var_max):
        ...
        for k in range(parto_count):
            ...
            for j in range(n_obj):
                ...
                for i in range(1, n - 1):
                    ...
                    d[idx[i], j] /= (costs[idx[-1], j] - costs[idx[0], j]).flatten()
